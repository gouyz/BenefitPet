...
class JCSingleSettingCell: UITableViewCell {
    ...
    private lazy var avatorButton: UIButton = {
        ...
        avatorButton.setBackgroundImage(UIImage.init(named: "com_icon_user_50"), for: .normal)
        avatorButton.addTarget(self, action: #selector(_clickAvator), for: .touchUpInside)
        return avatorButton
    }()
    private lazy var addButton: UIButton = {
        let addButton = UIButton()
        addButton.setBackgroundImage(UIImage.init(named:"com_icon_single_add"), for: .normal)
        addButton.setBackgroundImage(UIImage.init(named:"com_icon_single_add_per"), for: .highlighted)
